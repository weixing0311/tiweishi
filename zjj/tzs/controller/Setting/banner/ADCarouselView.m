...
{
    DLog(@"index--%ld--%@",(long)self.currentIndex,self.carouselImages);
...
//    self.pageControlView.numberOfPages = _imgs.count;
    [self.pageControlView setNumberOfPages:_imgs.count];
...
{
    if (!_pageControlView) {
        _pageControlView = [ADPageControlView pageControlViewWithFrame:CGRectZero dotsSize:CGSizeMake(7, 7) dotsMargin:10];
        self.pageControlView.allPageDotBackgroundColor = RGBACOLOR(0, 0, 0, .3);
        self.pageControlView.currentPageDotColor = RGBACOLOR(255, 255, 255, 1);
