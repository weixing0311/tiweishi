...
@implementation TZSTeamHeaderView
{
    int zhetimeout;
}
/*
...
-(void)updateTimeInVisibleCellsWithString:(int)str{
    if (str <=0) {
        self.lastTimeLabel.text = [NSString stringWithFormat:@"订单补充已结束"];
    }
    else
    {
        self.lastTimeLabel.text = [NSString stringWithFormat:@"订单补充剩余时间%@",[self getNowTimeWithString:zhetimeout]];
    }
}
